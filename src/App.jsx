...
function createDeck() {
	const d = [];
	for (let i = 0; i < 5; i++) for (const t of ALL_CARDS) d.push({ type: t, id: `${t}_${i}_${Math.random().toString(36).slice(2, 8)}` });
	return shuffle(d);
}
	...
	return (
		<div style={{ position: "absolute", inset: 0, overflow: "hidden", zIndex: 0 }}>
			<svg width="100%" height="100%" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", inset: 0 }}>
				...
				<text x="87" y="73" fontSize="7" fill="#f5f0e1" textAnchor="middle" fontFamily="serif" opacity="0.5">
					WANTED
				</text>
				<rect x="72" y="80" width="30" height="30" fill="#b8956a" rx="1" opacity="0.3" />
				<text x="87" y="120" fontSize="5" fill="#4a2a10" textAnchor="middle" fontFamily="serif" opacity="0.4">
					$500
				</text>
				...
				<text x="707" y="63" fontSize="7" fill="#f5f0e1" textAnchor="middle" fontFamily="serif" opacity="0.5">
					WANTED
				</text>
				<rect x="692" y="70" width="30" height="30" fill="#b8956a" rx="1" opacity="0.3" />
				<text x="707" y="110" fontSize="5" fill="#4a2a10" textAnchor="middle" fontFamily="serif" opacity="0.4">
					$1000
				</text>
				...
				{[300, 330, 355, 385, 420, 450, 480].map((x, i) => (
					<g key={i} opacity={0.4 + i * 0.05}>
						<rect
							...
							width={8 + (i % 2) * 4}
							height={22 - (i % 3) * 3}
							fill={["#2a5a2a", "#5a2a1a", "#8a6a20", "#1a3a5a", "#5a1a3a", "#3a5a2a", "#6a4a10"][i]}
							rx="1"
						/>
						<rect
							x={x}
			...
			{isCurrent && !eliminated && (
				<div
					style={{
						...
						textShadow: `0 0 6px ${T.goldBright}60`,
					}}
				>
					▼
				</div>
		...
		>
			<div
				style={{
					...
					borderRadius: 3,
					boxShadow: selected
						? `0 8px 20px rgba(0,0,0,0.5), 0 0 12px ${T.goldBright}40, inset 0 0 0 2px ${T.goldBright}`
						: highlight
							? `0 3px 8px rgba(0,0,0,0.4), 0 0 6px #2ecc7130, inset 0 0 0 1.5px #2ecc7180`
			...
			{faceUp ? (
				...
				>
					...
					<span style={{ position: "absolute", bottom: 2, right: 3, fontSize: small ? 8 : 10, color, fontFamily: "'Press Start 2P',monospace", transform: "rotate(180deg)" }}>
						{CARD_LABELS[type]}
					</span>
				</div>
			) : (
...
function PlayIndicator({ playerIdx, count, visible }) {
	...
	return (
		...
		>
			<span style={{ fontSize: 8, color: PT[playerIdx].primary, fontFamily: "'Press Start 2P',monospace", fontWeight: "bold" }}>{PLAYER_NAMES[playerIdx]}</span>
...
function RevolverAnimation({ playerIdx, onComplete, hit, spentChambers }) {
	...
	// bullet is at firing position (0) if hit, random elsewhere if not
	...
	return (
		...
		>
			{muzzleFlash && (
				<div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 40%,rgba(255,200,50,0.4),transparent 60%)", zIndex: 110, animation: "flashPulse 0.15s" }} />
			)}
			...
			<div style={{ fontSize: 13, color: PT[playerIdx].primary, fontFamily: "'Press Start 2P',monospace", marginBottom: 14, textShadow: "1px 1px 0 #000" }}>{PLAYER_NAMES[playerIdx]}</div>
			...
			<div style={{ minHeight: 50, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
				...
				{phase === "aim" && (
					<span style={{ fontSize: 12, color: T.redBright, fontFamily: "'Press Start 2P'", animation: "blink 0.4s infinite", textShadow: `0 0 10px ${T.redBright}` }}>. . .</span>
				)}
			...
			<div style={{ display: "flex", gap: 6, marginTop: 12, alignItems: "center" }}>
				{/* Chambres déjà tirées (grises) */}
				{Array.from({ length: spentChambers }, (_, i) => (
					<div key={`spent_${i}`} style={{ width: 14, height: 14, borderRadius: "50%", border: `1px solid ${T.textDim}30`, background: `${T.textDim}15`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 6, color: T.textDim }}>
						○
					</div>
				))}
				...
				{Array.from({ length: remaining }, (_, i) => (
					<div
						...
						style={{
							...
							height: 18,
							borderRadius: "50%",
							border: `2px solid ${i === 0 && phase === "result" ? (result === "dead" ? T.redBright : "#2ecc71") : `${T.textDim}40`}`,
							background: i === 0 && phase === "result" ? (result === "dead" ? T.redBright : "#2ecc71") : "transparent",
							display: "flex",
							alignItems: "center",
							justifyContent: "center",
							fontSize: 7,
							color: "#fff",
							boxShadow: i === 0 && phase === "result" && result === "dead" ? `0 0 10px ${T.redBright}` : "none",
							transition: "all 0.3s",
						}}
					>
				...
				{phase === "reveal" && chosen ? (
					<div style={{ animation: "cardReveal 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
						<div
							style={{
								...
								borderRadius: 6,
								boxShadow: `0 0 40px ${T.goldBright}30`,
								background: `radial-gradient(circle,${T.goldBright}08,transparent 70%)`,
							}}
						>
					...
					CARD_TYPES.map((type, i) => (
						<div
							key={type}
							style={{
								transform: i === spinIdx % CARD_TYPES.length ? "scale(1.25)" : "scale(0.75)",
								opacity: i === spinIdx % CARD_TYPES.length ? 1 : 0.2,
			...
			{roundCard && (
				<div
					style={{
						...
						border: `1px solid ${T.goldDim}50`,
						letterSpacing: 2,
						textShadow: `0 0 6px ${T.goldBright}40`,
					}}
				>
					★ {roundCard} ★
				</div>
...
export default function LiarsShot() {
	...
	// keep chambersRef in sync so trigRoulette always reads fresh values
	...
	const humanPlay = useCallback(() => {
		...
		setPlayInd(null);
		const p = players[0],
			cards = sel.map((i) => p.hand[i]),
			nh = p.hand.filter((_, i) => !sel.includes(i)),
			ok = legit(cards, roundCard);
		...
		if (nh.length === 0) {
			addLog("Main vide !");
			setTimeout(() => newRound(np, nextAlive(0, np)), 1200);
			return;
		}
	...
	const humanLiar = useCallback(() => {
		...
		setTimeout(() => {
			setRevealed(null);
			if (was) {
				addLog(`→ ${PLAYER_NAMES[lastP]} mentait !`);
				trigRoulette(lastP);
			} else {
				addLog("→ Vrai ! Vous tirez...");
				trigRoulette(0);
			}
		}, 1800);
	...
	const botPlay = useCallback(
		(bi, pl, rc) => {
			...
			const rCard = rc || roundCard,
				cnt = Math.min(1 + Math.floor(Math.random() * 2), b.hand.length);
			...
			const nh = b.hand.filter((c) => !ctp.includes(c)),
				ok = legit(ctp, rCard);
			...
			setPlayers(np);
			if (nh.length === 0) {
				addLog(`${b.name} main vide !`);
				setTimeout(() => newRound(np, nextAlive(bi, np)), 1200);
				return;
			}
	...
	const rouletteDone = useCallback(
		(died) => {
			const pi = rouletteP;
			setRouletteP(null);
			setPlayers((prev) => {
				...
				if (died) {
					...
					addLog(`${PLAYER_NAMES[pi]} survit ! (${CHAMBER_COUNT - chambersRef.current[pi] - 1} chambre${CHAMBER_COUNT - chambersRef.current[pi] - 1 > 1 ? "s" : ""} restante${CHAMBER_COUNT - chambersRef.current[pi] - 1 > 1 ? "s" : ""})`);
					setChambers((ch) => {
						const n = [...ch];
						n[pi]++;
						chambersRef.current = n;
						return n;
					});
				}
				setGs("playing");
	...
	const human = players[0],
		isMyTurn = cp === 0 && human?.alive,
		canLiar = isMyTurn && lastP !== null && lastP !== 0 && lastCards.length > 0;
		...
		return (
			...
			>
				...
				<div style={{ zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
					...
					>
						Posez la carte annoncée — ou bluffez.
						<br />
						Les <span style={{ color: T.goldBright }}>As ★</span> sont des jokers.
						<br />
						...
						{[10, 15, 20, 30].map((t) => (
							<button
								...
								style={{
									...
									fontSize: 8,
									padding: "5px 10px",
									background: timerDur === t ? T.rim : "transparent",
									color: timerDur === t ? T.goldBright : T.textDim,
									border: `1px solid ${timerDur === t ? T.gold : `${T.textDim}40`}`,
									cursor: "pointer",
									borderRadius: 2,
								}}
							>
					...
					<button
						...
						style={{
							...
							textShadow: `0 0 8px ${T.goldBright}40, 1px 1px 0 #000`,
							letterSpacing: 4,
		...
		>
			...
			<div
				style={{
					...
					borderBottom: `1px solid ${T.goldDim}15`,
					zIndex: 10,
				}}
			>
				...
				{timer !== null && (
					<span
						style={{
							fontSize: 12,
							fontFamily: "'Press Start 2P'",
							color: timer <= 5 ? T.redBright : T.goldBright,
							animation: timer <= 5 ? "blink 0.5s infinite" : "none",
							textShadow: timer <= 5 ? `0 0 8px ${T.redBright}` : "none",
						}}
					>
						⏱ {timer}s
					</span>
				...
				>
					<div
						style={{
							...
							opacity: 0.05,
							background:
								"url(\"data:image/svg+xml,%3Csvg width='4' height='4' viewBox='0 0 4 4' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 0h1v1H1zM3 2h1v1H3z' fill='%23000' fill-opacity='1'/%3E%3C/svg%3E\")",
							pointerEvents: "none",
						}}
					/>
						...
						return (
							...
							>
								<CowboySprite
									...
									size={38}
									eliminated={!p.alive}
									isCurrent={cp === i && !showAnn}
									emotion={emotes.some((e) => e.player === i) ? "smirk" : cp === i ? "suspicious" : "neutral"}
								/>
										...
										{Array.from({ length: CHAMBER_COUNT }, (_, ci) => (
											<div
												...
												style={{
													...
													height: 5,
													borderRadius: "50%",
													border: `1px solid ${ci < chambers[i] ? T.redBright : `${T.textDim}40`}`,
													background: ci < chambers[i] ? T.redBright : "transparent",
													boxShadow: ci < chambers[i] ? `0 0 3px ${T.redBright}60` : "none",
												}}
											/>
										))}
									</div>
								...
								{emotes
									...
									.map((e) => (
										<div
											key={e.id}
											style={{ position: "absolute", top: -22, fontSize: 22, animation: "fadeInUp 0.3s", zIndex: 50, filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.5))" }}
										>
											{e.emote}
										</div>
									))}
				...
				{log.map((l, i) => (
					<div
						...
						style={{
							fontSize: 6,
							fontFamily: "'Press Start 2P'",
							color: l.includes("═══") ? T.goldDim : l.includes("💀") ? T.redBright : T.textDim,
							lineHeight: 2,
					...
					<div style={{ display: "flex", gap: 4, justifyContent: "center", flexWrap: "wrap", marginBottom: 10 }}>
						{human.hand.map((c, i) => (
							<GameCard
								...
								highlight={c.type === roundCard || c.type === "As"}
								onClick={isMyTurn ? () => toggle(i) : undefined}
								small={human.hand.length > 7}
							/>
						))}
					</div>
					{isMyTurn ? (
						<div style={{ display: "flex", gap: 10, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
							{sel.length > 0 && (
								<button
									...
									style={{
										...
										fontSize: 17,
										padding: "6px 20px",
										background: "linear-gradient(135deg,#1a5c2a,#0d3a18)",
										color: "#2ecc71",
							...
							{canLiar && (
								<button
									...
									style={{
										...
										fontSize: 17,
										padding: "6px 20px",
										background: `linear-gradient(135deg,${T.rim},#5a2510)`,
										color: T.redBright,
										border: `2px solid ${T.redBright}80`,
										cursor: "pointer",
										borderRadius: 3,
										boxShadow: "0 3px 12px rgba(0,0,0,0.4)",
										letterSpacing: 2,
							...
							<div style={{ position: "relative" }}>
								<button
									...
									style={{ fontSize: 18, padding: "4px 10px", background: "transparent", color: T.text, border: `1px solid ${T.textDim}30`, cursor: "pointer", borderRadius: 3 }}
								>
									😏
								</button>
								{showEm && (
									...
									>
										{EMOTES.map((e) => (
											<span
												key={e}
												style={{ fontSize: 18, cursor: "pointer", transition: "transform 0.15s" }}
												onClick={() => {
													emote(0, e);
													setShowEm(false);
												}}
			...
			{gs === "roulette" && rouletteP !== null && <RevolverAnimation playerIdx={rouletteP} hit={rouletteHit} spentChambers={chambers[rouletteP]} onComplete={rouletteDone} />}
		</div>
	);
}
